...
function getDays(
  ...
  const { firstDayOfWeek, firstWeekday, isoWeeknumbers, weeknumbers, numDays, numWeeks } =
    monthComps;
  const prevMonthDaysToShow =
    firstWeekday + (firstWeekday < firstDayOfWeek ? daysInWeek : 0) - firstDayOfWeek;
  let prevMonth = true;
  let thisMonth = false;
  let nextMonth = false;
  let position = 0;
...
function getWeeks(
  ...
  result.forEach((week) => {
    ...
    if (fromDay.month === toDay.month) {
      ...
      week.title = `${locale.formatDate(fromDay.date, 'MMM')} - ${locale.formatDate(
        toDay.date,
        'MMM YYYY'
      )}`;
    } else {
      week.title = `${locale.formatDate(fromDay.date, 'MMM YYYY')} - ${locale.formatDate(
        toDay.date,
        'MMM YYYY'
      )}`;
    }
